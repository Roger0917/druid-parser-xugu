...
public class XuguStatementParser extends SQLStatementParser{
    ...
    @Override
    public XuguCreateTableParser getSQLCreateTableParser() {
        return new XuguCreateTableParser(lexer);
    }

    @Override
    protected void parseInsert0_hinits(SQLInsertInto insertStatement) {
        if (insertStatement instanceof XuguInsertStatement) {
            XuguInsertStatement stmt = (XuguInsertStatement) insertStatement;
    ...
    public void parseStatementList(List<SQLStatement> statementList, int max, SQLObject parent) {
        for (;;) {
            ...
            if (lexer.token() == (Token.SELECT)) {
                SQLStatement stmt = new SQLSelectStatement(new XuguSelectParser(this.exprParser).select(), DbType.xugu);
                //SQLStatement stmt = new SQLSelectStatement(DbType.xugu);
            ...
            if (lexer.token() == (Token.UPDATE)) {
                ...
                //SQLStatement stmt = new SQLUpdateStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == (Token.CREATE)) {
                ...
                //SQLStatement stmt = new SQLCreateTableStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == Token.INSERT) {
                ...
                //SQLStatement stmt = new XuguInsertStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == (Token.DELETE)) {
                ...
                //SQLStatement stmt = new XuguDeleteStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == Token.ALTER) {
                ...
                //SQLStatement stmt = new XuguAlterTriggerStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == Token.BEGIN
                ...
                //SQLBlockStatement stmt = new SQLBlockStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
           ...
           /* if (lexer.token() == Token.LOCK) {
            ...
            if (lexer.token() == Token.EXCEPTION) {
                XuguExceptionStatement stmt = this.parseException();
                stmt.setParent(parent);
            ...
            if (lexer.token() == Token.CONTINUE) {
                lexer.nextToken();
                XuguContinueStatement stmt = new XuguContinueStatement();
            ...
            if (lexer.token() == Token.EXPLAIN) {
                XuguExplainStatement stmt = this.parseExplain();
                //XuguExplainStatement stmt = new XuguExplainStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
...
//                    Lexer.SavePoint savePoint = lexer.mark();
            ...
            if (lexer.token() == Token.FOR) {
                XuguForStatement forStatement = this.parseFor();
            ...
            if (lexer.token() == Token.GOTO) {
                ...
                XuguGotoStatement stmt = new XuguGotoStatement(label);
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == Token.DROP) {
                SQLDropStatement stmt = new SQLDropFunctionStatement();
                stmt.setParent(parent);
                statementList.add(stmt);
                continue;
            }
            ...
            if (lexer.token() == Token.WHEN
                    && parent instanceof XuguExceptionStatement.Item) {
                break;
            }
    ...
    public SQLStatement parseExecute() {
        ...
        if (lexer.token() == Token.IMMEDIATE) {
            ...
            XuguExecuteImmediateStatement stmt = new XuguExecuteImmediateStatement();
    ...
    public SQLStatement parseRaise() {
        lexer.nextToken();
        XuguRaiseStatement stmt = new XuguRaiseStatement();
        if (lexer.token() != Token.SEMI) {
...
//        }
    ...
    public SQLStatement parserAlter() {
        ...
        if (lexer.token() == Token.SESSION) {
            ...
            lexer.nextToken();
            XuguAlterTriggerStatement stmt = new XuguAlterTriggerStatement();
            stmt.setName(this.exprParser.name());
        ...
        for (;;) {
            if (lexer.identifierEquals(FnvHash.Constants.ADD)) {
                ...
                if (lexer.token() == Token.LPAREN) {
                    ...
                    XuguConstraint constraint = ((XuguExprParser) this.exprParser).parseConstaint();
    ...
    private XuguExceptionStatement parseException() {
        accept(Token.EXCEPTION);
        XuguExceptionStatement stmt = new XuguExceptionStatement();

        for (;;) {
            accept(Token.WHEN);
            XuguExceptionStatement.Item item = new XuguExceptionStatement.Item();
    ...
    @Override
    public XuguCreateIndexStatement parseCreateIndex(boolean acceptCreate) {
        if (acceptCreate) {
            accept(Token.CREATE);
        }

        XuguCreateIndexStatement stmt = new XuguCreateIndexStatement();
    ...
    public SQLUpdateStatement parseUpdateStatement() {
        return new XuguUpdateParser(this.lexer).parseUpdateStatement();
    }
    ...
    public SQLStatement parseCreatePackage() {
        ...
        XuguCreatePackageStatement stmt = new XuguCreatePackageStatement();
        ...
        accept(Token.END);
       /* if(lexer.token()==Token.SUBTYPE){
            accept(Token.SUBTYPE);
            return stmt;
        }else{
            accept(Token.END);
        }*/
    ...
    public SQLStatement parseCreateSynonym() {
        XuguCreateSynonymStatement stmt = new XuguCreateSynonymStatement();
        accept(Token.CREATE);
    ...
    public SQLStatement parseCreateType() {
        XuguCreateTypeStatement stmt = new XuguCreateTypeStatement();
        accept(Token.CREATE);
