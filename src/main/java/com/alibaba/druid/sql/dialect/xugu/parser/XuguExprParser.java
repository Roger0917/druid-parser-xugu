        ...
        if (lexer.token() == Token.INTERVAL) {
            ...
            if (lexer.identifierEquals("YEAR")) {
                ...
                if(lexer.token()!=Token.TO){
                    ...
                    interval.getArguments().stream().forEach(sqlExpr -> intervalYearToMonth.addArgument(sqlExpr));
                ...
                if(lexer.token()!=Token.TO){
                    ...
                    accept(Token.TO);
                    try {
                        acceptIdentifier("HOUR");
                        interval.getArguments().stream().forEach(sqlExpr -> intervalDayToHour.addArgument(sqlExpr));
                        return intervalDayToHour;
                    }catch (ParserException e){
                        //dayToMinute和dayToSecond类型解析时会进入此catch
                        //e.printStackTrace();
                    }
                    try{
                        acceptIdentifier("MINUTE");
                        interval.getArguments().stream().forEach(sqlExpr -> intervalDayToMinute.addArgument(sqlExpr));
                        return intervalDayToMinute;
                    }catch (ParserException e){
                        //dayToSecond类型解析时会进入此catch
                        //e.printStackTrace();
                    }
                ...
                if(lexer.token()!=Token.TO){
                    ...
                    try{
                        acceptIdentifier("MINUTE");
                        interval.getArguments().stream().forEach(sqlExpr -> intervalHourToMinute.addArgument(sqlExpr));
...
//        methodName = methodName.toUpperCase();
    ...
    @Override
    public XuguSelectParser createSelectParser() {
...
//        }
    ...
    /*public SQLObject parseOpaque() {
        ...
        XuguLobStorageClause clause = new XuguLobStorageClause();
    ...
    public XuguLobStorageClause parseLobStorage() {
        lexer.nextToken();
    ...
    @Override
    public XuguConstraint parseConstaint() {
        XuguConstraint constraint = (XuguConstraint) super.parseConstaint();
    ...
    protected SQLPartition parsePartition() {
        ...
        for (;;) {
            ...
            if (lexer.token() == Token.LOB) {
                XuguLobStorageClause lobStorage = this.parseLobStorage();
    ...
    public void parseSegmentAttributes(XuguSegmentAttributes attributes) {
        for (;;) {
            if (lexer.token() == Token.TABLESPACE) {
                lexer.nextToken();
